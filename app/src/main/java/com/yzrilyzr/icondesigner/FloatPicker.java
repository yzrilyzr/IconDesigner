...
public class FloatPicker extends MView
{
	...
	private int mode=0;
	private FloatPickerEvent ifc=null;
	private float width;
	...
	public void onTouchEvent(MotionEvent event)
	{
		...
		else if(2*r<ot&&ot<3*r&&mode==0)
		{
			...
			float c2x=(float)(2.5*r*Math.cos(degree-2.0/9.0*Math.PI));
			float c2y=(float)(2.5*r*Math.sin(degree-2.0/9.0*Math.PI));
			if(Math.sqrt(Math.pow(c+c1x-x,2)+Math.pow(c+c1y-y,2))<r/2)
			{setValue((float)(value+=0.01));mode=1;}
			else if(Math.sqrt(Math.pow(c+c2x-x,2)+Math.pow(c+c2y-y,2))<r/2)
			{setValue((float)(value-=0.01));mode=1;}
			else mode=2;
		}
		if(mode!=1)
